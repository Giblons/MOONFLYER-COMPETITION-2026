...
def load_ncnn_model(pt_path, task_type=None):
    """Loads NCNN model, exporting it first if necessary."""
    if not os.path.exists(pt_path):
        print(f"❌ Original .pt model not found: {pt_path}")
        return None
    
    ncnn_dir = pt_path.replace(".pt", "_ncnn_model")
    if not os.path.exists(ncnn_dir):
        print(f"\n--- Exporting {pt_path} to NCNN format ---")
        model = YOLO(pt_path, task=task_type)
        model.export(format="ncnn")
    
    print(f"Loading NCNN model from {ncnn_dir}")
    return YOLO(ncnn_dir, task=task_type)

# ==========================================
# test_vcor.py functions (NCNN)
# ==========================================
def test_on_dataset():
    print("\n--- Testing on VCOR dataset ---")
    print("Evaluating Model 1 (VisDrone Detection Base) on Test Set...")
    model1 = load_ncnn_model(CLASS_MODEL1_PATH, task_type='classify')
    if model1:
    ...
    model2 = load_ncnn_model(CLASS_MODEL2_PATH, task_type='classify')
    if model2:
...
def test_each_color():
    ...
    model = load_ncnn_model(CLASS_MODEL1_PATH, task_type='classify')
    if not model:
        return
...
def run_video_tests():
    print("\n--- Running video annotation tests (NCNN) ---")
    det_model = load_ncnn_model(DET_MODEL_PATH, task_type='detect')
    model1    = load_ncnn_model(CLASS_MODEL1_PATH, task_type='classify')
    model2    = load_ncnn_model(CLASS_MODEL2_PATH, task_type='classify')

    if not all([det_model, model1, model2]):
        print("❌ Could not load all models in NCNN format.")
        return
    ...
    for vid_path in VIDEOS:
        ...
        out1 = f"output_ncnn_{dataset_name}_model1.mp4"
...
def test_tracking_models():
    ...
    for i in range(min(len(model_paths), len(VIDEOS))):
        ...
        model = load_ncnn_model(m_path, task_type='detect')
        if not model:
            continue

        try:
            print("Starting inference...")
            results = model.track(source=v_path, show=True)
            print(f"✅ Finished testing Model #{i+1} on Video #{i+1}")
        except Exception as e:
            print(f"❌ An error occurred during tracking: {e}")

if __name__ == "__main__":
    print("Select a test to run (NCNN models):")
